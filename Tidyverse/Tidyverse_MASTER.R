...
# Generate sex
sex <- c(rep("Male", 16), rep("Female", 4))
...
generate_dates <- function(n, start_date, end_date) {
  as.Date(runif(n, min = as.numeric(start_date), max = as.numeric(end_date)), origin = "1970-01-01")
}

# Generate visits and viral loads
data <- data.frame()

for (id in subject_ids) {
  num_visits <- sample(1:4, 1)
  visit_dates <- sort(generate_dates(num_visits, start_date, end_date))
  
  # Handle cases where num_visits is odd
  half_visits <- num_visits %/% 2
  remainder_visits <- num_visits %% 2
  
  # Generate viral loads
  viral_loads <- c(runif(half_visits, 20, 200), runif(half_visits + remainder_visits, 250, 10000))
  
  # If num_visits is 1, we need one value, so we sample from the combined range
  if (num_visits == 1) {
    viral_loads <- sample(c(runif(1, 20, 200), runif(1, 250, 10000)), 1)
  } else {
    # Shuffle viral loads to ensure they are not in any particular order
    viral_loads <- sample(viral_loads)
  }
  
  temp_data <- data.frame(
    SubjectID = rep(id, num_visits),
    Sex = rep(sex[id], num_visits),
    Age = rep(age[id], num_visits),
    Date = visit_dates,
    ViralLoad = viral_loads
  )
...
data %>%
  mutate(ViralLoad = round(ViralLoad),
         ageMonths = Age*12,
         dob = Date %m-% months(ageMonths)) %>%
  group_by(SubjectID) %>%
  arrange(Date) %>%
  slice(1) %>%
  select(SubjectID, dob) -> dobs

# Merge dates of birth and full dataset
data %>%
  select(SubjectID, Sex, Date, ViralLoad) -> dataTemp

dat = merge(dataTemp, dobs, by = 'SubjectID', all = T)

# Calculate age
dat %>%
  mutate(Age = as.numeric(substrRight(round((Date - dob)/365.25), 4))) %>%
  select(SubjectID, Date, Age, Sex, ViralLoad) -> dat

# Save df
# write.csv(dat, 'TidyverseTutorialData.csv', row.names = F)
